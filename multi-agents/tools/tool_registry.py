...
AVAILABLE_TOOLS: List[ToolDefinition] = [
    ...
    ToolDefinition(
        ...
        parameters={
            ...
            "properties": {
                ...
                "city": {
                    "type": "string",
                    "description": "城市信息，非必须，例如：'北京'、'上海'、'苏州'"
                }
            },
            "required": ["address"]
        },
    ...
    ToolDefinition(
        name="gaode_regeo",
        description="逆地理编码：将一个高德经纬度坐标转换为行政区划地址信息，返回省市区等地址组成信息。",
        parameters={
            "type": "object",
            "properties": {
                "location": {
                    "type": "string",
                    "description": "位置经纬度，格式：'经度,纬度'，例如：'120.619585,31.299379'"
                }
            },
            "required": ["location"]
        },
        tool_type="mcp",
        server_name="Gaode Server",
        mcp_tool_name="maps_regeo"
    ),

    ToolDefinition(
        name="gaode_ip_location",
        description="IP 定位：根据用户输入的 IP 地址定位所在省、市和城市编码。",
        parameters={
            "type": "object",
            "properties": {
                "ip": {
                    "type": "string",
                    "description": "IP 地址，例如：'114.247.50.2'"
                }
            },
            "required": ["ip"]
        },
    ...
    ToolDefinition(
        ...
        description="骑行路径规划：规划骑行通勤方案，最大支持 500km。",
        parameters={
            "type": "object",
            "properties": {
                "origin": {
                    "type": "string",
                    "description": "起点经纬度，格式：'经度,纬度'"
                },
                "destination": {
                    "type": "string",
    ...
    ToolDefinition(
        name="gaode_walking",
        description="步行路径规划：规划 100km 以内的步行通勤方案。",
        parameters={
            "type": "object",
            "properties": {
                "origin": {
    ...
    ToolDefinition(
        name="gaode_transit",
        description="公交路径规划：综合火车、公交、地铁等公共交通方式；跨城场景必须传起点城市与终点城市。",
        parameters={
            "type": "object",
            "properties": {
                "origin": {
    ...
    ToolDefinition(
        name="gaode_distance",
        description="距离测量：测量两个经纬度坐标之间的距离，并返回距离和时间等信息。",
        parameters={
            "type": "object",
            "properties": {
                "origin": {
    ...
    ToolDefinition(
        ...
        description="周边搜索：根据关键词和中心点坐标，搜索指定半径范围内的 POI 地点信息。",
        parameters={
            "type": "object",
            "properties": {
                "keywords": {
                    "type": "string",
                    "description": "搜索关键词，例如：'餐厅'、'景点'、'酒店'"
                },
                "location": {
                    "type": "string",
                ...
                "radius": {
                    "type": "string",
                    "description": "搜索半径，非必须，单位米，例如：'1000'"
                }
            },
    ...
    ToolDefinition(
        name="gaode_place_detail",
        description="详情搜索：根据关键词搜索或周边搜索获取到的 POI ID 查询地点详细信息。",
        parameters={
            "type": "object",
            "properties": {
                "id": {
                    "type": "string",
                    "description": "关键词搜索或周边搜索获取到的 POI ID"
                }
            },
            "required": ["id"]
        },
        tool_type="mcp",
        server_name="Gaode Server",
        mcp_tool_name="maps_place_detail"
    ),
