...
分类标准：
- conversation: 对话类查询（问候、感谢、闲聊、对上次回答的反馈评价等）。不含目的地/行程要素。
- travel: 旅游规划类查询。**只要提到目的地、出发地、天数、预算、日期、景点、天气、美食、车票、酒店等任一旅行要素，就是 travel**。包括规划行程、修改旅行需求（"预算改成3000"）、询问攻略/推荐/天气等。

判断优先级：提到任何旅行要素 → travel；纯粹社交对话 → conversation。
