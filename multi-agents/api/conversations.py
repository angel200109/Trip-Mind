...
async def create_conversation(req: CreateConversationRequest):
    service = get_conversation_service()
    conversation = await service.create_conversation(title=req.title)
    return ApiResponse(data=conversation.model_dump())
...
async def delete_conversation(session_id: str):
    service = get_conversation_service()
    await service.delete_conversation(session_id)
...
async def update_title(session_id: str, req: UpdateTitleRequest):
    service = get_conversation_service()
    await service.update_title(session_id, req.title)
    return ApiResponse(data=True)
