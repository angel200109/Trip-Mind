...
const regenerateMessage = async () => {
  try {
    ...
    ElMessage.error({ message: "重新生成失败", duration: 1000 });
  }
};
</script>
    ...
    <div v-if="message.role === 'assistant'" class="send-text ai-response">
      <template v-if="message.progress">
        <div class="ai-status">
          ...
          <span v-if="message.content" class="ai-status-text">
            {{ message.content }}
          </span>
        </div>
      </template>
      <MarkdownRenderer v-else :content="message.content as string" ></MarkdownRenderer>
      ...
      <button
        v-if="!message.progress && !store.prohibit && isLast"
        class="copy-btn"
        type="button"
