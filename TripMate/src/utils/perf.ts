type StreamPerfReport = {
  ...
  uiCommitTotalMs: number;
  uiCommitAvgMs: number;
  uiCommitMaxMs: number;
  uiFrameTotalMs: number;
  uiFrameAvgMs: number;
  uiFrameMaxMs: number;
...
export const perfTracker = {
  ...
  recordUiUpdate(commitMs: number, frameMs: number) {
    if (!activeSession) return;
    activeSession.uiUpdateCount += 1;
    activeSession.uiCommitTotalMs += commitMs;
    activeSession.uiCommitMaxMs = Math.max(
      activeSession.uiCommitMaxMs,
      commitMs,
    );
    activeSession.uiFrameTotalMs += frameMs;
    activeSession.uiFrameMaxMs = Math.max(
      activeSession.uiFrameMaxMs,
      frameMs,
    );
  },
  ...
  endStream(status: StreamPerfReport["status"] = "completed") {
    ...
    const report: StreamPerfReport = {
      ...
      uiUpdateCount: activeSession.uiUpdateCount,
      uiCommitTotalMs: activeSession.uiCommitTotalMs,
      uiCommitAvgMs:
        activeSession.uiUpdateCount > 0
          ? activeSession.uiCommitTotalMs / activeSession.uiUpdateCount
          : 0,
      uiCommitMaxMs: activeSession.uiCommitMaxMs,
      uiFrameTotalMs: activeSession.uiFrameTotalMs,
      uiFrameAvgMs:
        activeSession.uiUpdateCount > 0
          ? activeSession.uiFrameTotalMs / activeSession.uiUpdateCount
          : 0,
      uiFrameMaxMs: activeSession.uiFrameMaxMs,
